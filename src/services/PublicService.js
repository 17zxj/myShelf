...
export default class PublicService {
   ...
   * 根据点位查询地点
   */ 
  static lonLatResult(lon, lat, callback) {
    let url = `http://api.tianditu.gov.cn/geocoder?postStr={'lon':${lon},'lat':${lat},'ver':1}&type=geocode&tk=13b4890427f17c692f9efed46b98db81`;
    fetch(url)
      .then(resp => {
        return resp.json();
      })
   ...
   */
  static addressSearch(address, callback) {
    let url = `https://restapi.amap.com/v3/geocode/geo?key=cbfe363443683fff47d4f60c0899e0ca&city=杭州&address=${address}`;
        ...
        if (info === "OK" && status === "1" && geocodes.length) {
          geocodes.map(item => {
            ...
            if (location) {
              ...
              item.TDT = PublicService.onConvertFrom(
                location[0] * 1,
                location[1] * 1
              )
            }
  ...
  // 根据点位查询地址
  static lonLatSearch({lon, lat}, callback) {
    let url = `http://api.tianditu.gov.cn/geocoder?postStr={'lon':${lon},'lat':${lat},'ver':1}&type=geocode&tk=13b4890427f17c692f9efed46b98db81`;
    fetch(url).then(resp => resp.json()).then(
      data => {
        let result = {};
