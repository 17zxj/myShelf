...
class Container extends Component {
  ...
  menuImport = () => {
    const t = this;
    let { importBtnUrl, upSuccess, templateUrl, onTemplate } = t.props;
    const uploadProps = {
      name: 'file',
      action: importBtnUrl,
      accept: '.xls, .xlsx',
      ...
      onSuccess(info) {
        if (info.code !== 1) {
          message.error(info.err || info.message);
        } else {
  ...
  render() {
    ...
    let { heightAuto, style, extraBtn, addBtn, addBtnText, headerShow = true, exportBtn, className } = t.props;

    return (
      <div className={`${styles.container} ${className}`} style={{ height: heightAuto ? containerHeight : null, ...style }}
           id="container">
        {
          headerShow &&
          <div className={styles.header} id='PublicContainerHeader'>
            ...
            {/* 导出本页/导出全部 */}
            {
              ...
              <Popconfirm
                ...
                placement="bottom">
                {
                  t.renderBtnItem({ text: '导出', color: '#EC9D38', icon: 'icondaochu' })
                }
              </Popconfirm>
            }
            ...
            {/* 导入 */}
            {
              ...
              <Dropdown overlay={t.menuImport} trigger={['click']}>
                {
                  t.renderBtnItem({ text: '导入', color: '#000000', icon: 'iconwenjiandaoru' })
                }
              </Dropdown>
            }
            ...
            {
              t.props.deleteBtn &&
              t.renderBtnItem({ text: '删除', onClick: t.props.deleteBtn, color: '#F12E28', icon: 'icon-shanchu1' })
            }
