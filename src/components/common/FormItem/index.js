...
import {
  ...
  Button, Modal, Icon, Cascader, AutoComplete
} from 'antd';
...
function getDefaultProps(val, props) {
  ...
  // 输入型控件
  if (['input', 'textArea', 'number', 'autoComplete'].includes(type)) {
    defaultVal.placeholder = !disabled ? `请输入${label}` : '';
  }

  // 选择型控件
  if (['timePicker', 'datePicker', 'yearPicker', 'monthPicker', 'select', 'treeSelect', 'cascade'].includes(type)) {
    defaultVal.placeholder = !disabled ? `请选择${label}` : '';
  }

  // 上传型控件
  if (['imgUpload', 'fileUpload', 'videoUpload', 'voiceUpload'].includes(type)) {
    defaultVal.name = 'multipartFile';
    defaultVal.action = '/zuul/dfs/file/upload?curPath=/xihu';
...
class FormItem extends Component {
  ...
  renderInput = (item) => {
    return (
      <Input {...getDefaultProps(item, this.props)} style={item.wrapStyle} />
    );
  };
  ...
  renderAutoComplete = (item) => {
    ...
    return (
      <AutoComplete {...getDefaultProps(item, this.props)} >
        {
  ...
  renderInputPwd = (item) => {
    return (
      <Input.Password {...getDefaultProps(item, this.props)} />
    )
  }
  ...
  renderImgUpload = (item) => {
    ...
    return (
      <Upload {...props} ref={ref => t.uploadImg = ref}>
        {uploadButton}
        <Modal
  ...
  render() {
    const t = this;
    let { formItem = {}, form = {}, formItemLayout = {} } = t.props;
    let { getFieldDecorator } = form;
    let { type, label, paramName, key, initialValue, required, rules = [] } = formItem;
    // 处理上传类组件
    let itemConfig = {
      initialValue,
      rules,
    ...
    const FORM = {
      ...
      'autoComplete': t.renderAutoComplete,             // 输入框
      'password': t.renderInputPwd,       // 输入密码框
      ...
      'rangeDatePicker': t.renderRangeDatePicker  //日期时间选择框
    };
    return (
      <Form.Item label={label} {...formItemLayout} className={styles.commonForm}>
        {
          FORM[type] &&
