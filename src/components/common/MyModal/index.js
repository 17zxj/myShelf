...
class MyModal extends Component {
  ...
  // 模态框拖拽事件
  modalDraggable = () => {
    const modalPropsWidth = this.props.width;
    ...
    for (let i = 0; i < modalHeaderList.length; i++) {
      ...
      let modalWidth = header.offsetWidth?(header.offsetWidth / 2):(modalPropsWidth/2);
      // 设置模态框的位置
      if (header) {
  ...
  render() {
    let t = this;
    let {className,footerShow} = t.props;
    return (
      <Modal
          maskClosable={false}
          className={`${styles.commonModal} ${className}`}
          footer={footerShow?t.renderFooter():false}
          getContainer={false}  
          {...t.props}
        >
          {t.props.children}
